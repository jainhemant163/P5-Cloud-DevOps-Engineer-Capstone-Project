    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'dockerhub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
                    sh '''
                        docker build -t jainhemant163/capstone-devops .
                    '''
                }
            }
        }
            ...
            steps {
                withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'dockerhub', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD']]){
                    sh '''
                        ...
                        docker push jainhemant163/capstone-devops
                    '''
                }
            }
        }
        ...
        stage('Set kubectl context') {
            steps {
                withAWS(region:'us-west-2', credentials:'aws-devops') {
                    sh '''
                        kubectl config use-context arn:aws:eks:us-west-2:109841304978:cluster/capstone-cloud-devops-cluster
                    '''
                }
            }
        }
